...
([канон-flow](neomarket-canon/flows/b2c-cart-flows.md#b2c-7-subscriptions)).
...
| POST | `/api/v1/favorites/{product_id}/subscribe` | Создание подписки на `IN_STOCK` / `PRICE_DOWN` |
| DELETE | `/api/v1/favorites/{product_id}/subscribe` | Идемпотентная отписка от уведомлений по товару |
...
├── subscriptions.py    # use-case/repository model подписок
