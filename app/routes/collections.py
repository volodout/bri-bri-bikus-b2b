...
from app.collections import (
    CollectionService,
    validate_collection_products_pagination,
    validate_collections_pagination,
)
...
@router.get("/api/v1/collections/{collection_id}/products")
async def get_collection_products(request: Request, collection_id: str) -> dict:
    collection_id = validate_uuid(collection_id, field="collection_id")
    limit, offset = validate_collection_products_pagination(
        request.query_params.get("limit"),
        request.query_params.get("offset"),
    )
    service = get_collection_service(request)
